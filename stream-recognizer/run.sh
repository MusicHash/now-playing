...
# REDIS_URI / MYSQL_URI from .env (and repo root .env for MYSQL) — abort if dependency is down
...
mysql_uri_from_env() {
    ...
    for f in "$ENV_FILE" "$SCRIPT_DIR/../.env"; do
        [[ -f "$f" ]] || continue
        v="$(grep -m1 '^MYSQL_URI=' "$f" 2>/dev/null | sed 's/^MYSQL_URI=//' | sed 's/[[:space:]]*$//' | sed 's/^["'\'']//' | sed 's/["'\'']$//' | tr -d '\r')"
        if [[ -n "$v" ]]; then
            printf '%s' "$v"
            return 0
        fi
...
mysql_is_up() {
    ...
    MYSQL_CHK_HOST=""
    MYSQL_CHK_PORT=""
    parse_mysql_host_port "$uri" || return 1
    (echo >/dev/tcp/${MYSQL_CHK_HOST}/${MYSQL_CHK_PORT}) 2>/dev/null
}
...
MYSQL_URI_VAL="$(mysql_uri_from_env)"
if [[ -z "$MYSQL_URI_VAL" ]]; then
    red "✗ MYSQL_URI is not set in $ENV_FILE or $(cd "$SCRIPT_DIR/.." && pwd)/.env"
    exit 1
fi
if ! mysql_is_up "$MYSQL_URI_VAL"; then
    parse_mysql_host_port "$MYSQL_URI_VAL" 2>/dev/null || true
    red "✗ MySQL is not listening on MYSQL_URI host:port (${MYSQL_CHK_HOST:-?}:${MYSQL_CHK_PORT:-?}) — start MySQL first."
    exit 1
fi
