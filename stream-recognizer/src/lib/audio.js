...
/**
 * Stricter check before recognition APIs: off-air / dead streams often pass {@link analyzePcmGates}
 * because encoder noise keeps RMS and "active" frame counts above silence thresholds.
 *
 * @param {Buffer} pcmS16le
 * @param {object} opts
 * @param {number} [opts.silenceDb=-45] frame is "silent" if its RMS dBFS is below this
 * @param {number} [opts.minPeakDb=-42] empty if peak dBFS across the clip is below this (no loud enough transient)
 * @param {number} [opts.maxSilentFrameRatio=0.94] empty if this fraction or more of frames are silent
 * @returns {{ empty: boolean, reason: 'peak_too_low'|'mostly_silent_frames'|null, peakDb: number, meanDb: number, silentFrameRatio: number, frameCount: number }}
 */
export function analyzePcmPreRecognitionEmpty(pcmS16le, opts = {}) {
    const silenceDb = opts.silenceDb ?? -45;
    const minPeakDb = opts.minPeakDb ?? -42;
    const maxSilentFrameRatio = opts.maxSilentFrameRatio ?? 0.94;
    const frameMs = opts.frameMs ?? 30;
    ...
    for (let i = 0; i + frameSamples <= samples.length; i += frameSamples) {
        ...
        const rms = Math.sqrt(s / frameSamples);
        const frameDb = 20 * Math.log10(rms / 32768 + 1e-12);
        sumSqAll += s;
        nFrames++;
        if (frameDb < silenceDb) {
            silentFrames++;
        }
    }

    const globalRms = Math.sqrt(sumSqAll / samples.length);
    const meanDb = 20 * Math.log10(globalRms / 32768 + 1e-12);
    const peakDb = 20 * Math.log10(maxAbs / 32768 + 1e-12);
    const silentFrameRatio = nFrames ? silentFrames / nFrames : 1;

    let empty = false;
    /** @type {'peak_too_low'|'mostly_silent_frames'|null} */
    let reason = null;
    if (peakDb < minPeakDb) {
        empty = true;
        reason = 'peak_too_low';
    } else if (silentFrameRatio >= maxSilentFrameRatio) {
        empty = true;
        reason = 'mostly_silent_frames';
    }
