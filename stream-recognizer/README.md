...
Logs include `capturePath`, `fingerprintPrefix`, and **`order`** (provider list). The temp file is deleted after each tick unless you set **`DEBUG_CAPTURE_DIR`** in `.env`; then a copy is kept as `{stationId}-{timestamp}.wav` in that folder.
...
| `DEBUG_CAPTURE_DIR` | If set, on AcoustID “no match” copy the captured WAV here for listening |
